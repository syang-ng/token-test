...
contract TransferAndFundTrader is Script {
    ...
    function run() public {
        _transferAndFund(
            vm.envAddress("TRADER_ADDRESS"), vm.envAddress("NEW_OWNER_ADDRESS"), vm.envUint("DEPLOYER_PRIVATE_KEY")
        );
    }

    function _transferAndFund(address target, address newOwner, uint256 ownerKey) internal {
        ...
        require(ownerKey != 0, "Local signing key is required");
        address sender = vm.addr(ownerKey);
        VulnerableTrader trader = VulnerableTrader(target);
        address currentOwner = trader.owner();
        require(currentOwner == sender || currentOwner == newOwner, "Local signer cannot transfer current ownership");
        ...
        require(IERC20(USDC).balanceOf(sender) >= AMOUNT, "Local signer needs 1 USDC");
        require(IERC20(EURC).balanceOf(sender) >= AMOUNT, "Local signer needs 1 EURC");
        require(sender.balance > 0, "Local signer needs Sepolia ETH for gas");

        vm.startBroadcast(ownerKey);
        if (currentOwner != newOwner) trader.transferOwner(newOwner);
